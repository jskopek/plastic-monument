...
import Monument from './future/monument.js';
import Pillar from './future/pillar.js';
...
pillarData.forEach((pillarRow) => { yearScroller.add(pillarRow.year, 0.05); })
...
timelineValues.forEach((index) => { timeline.add(index, .04, () => { return index; }); })
...
document.querySelectorAll('.page-content > h2').forEach((el) => {
    var watcher = scrollMonitor.create(el);
    watcher.enterViewport(() => { 
        var index = _.indexOf(pillarData, _.find(pillarData, (p) => { return p.year == el.innerText; }))
